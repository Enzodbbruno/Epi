...
const CaseNotificationModule = {
        ...
        <form id="notification-form-${id}" class="notification-form">
          ...
          <div style="display: flex; gap: 10px; flex-wrap: wrap;">
            <div class="form-group" style="flex: 1; min-width: 120px;">
              <label for="patient-cep-${id}">
                <i class="fas fa-map-pin"></i>
                CEP:
              </label>
              <div style="display: flex; gap: 5px;">
                <input type="text" id="patient-cep-${id}" placeholder="00000-000" maxlength="9" onblur="if(typeof fetchCepForLocation === 'function') fetchCepForLocation('${id}')">
                <button type="button" class="btn btn-outline small" onclick="if(typeof fetchCepForLocation === 'function') fetchCepForLocation('${id}')" title="Buscar CEP" style="padding: 0 12px; border-radius: 8px;">
                  <i class="fas fa-search"></i>
                </button>
              </div>
            </div>
            <div class="form-group" style="flex: 2; min-width: 200px;">
              ...
              <input type="text" id="patient-location-${id}" placeholder="Informe o bairro" required>
            </div>
          </div>
          <div class="form-group">
            <label for="health-unit-${id}">
              <i class="fas fa-hospital-alt"></i>
              Postinho / UBS Atendente:
            </label>
            <select id="health-unit-${id}" required style="width: 100%; padding: 12px; border: 1px solid #e0e0e0; border-radius: 8px; font-size: 1rem; color: #333; outline: none;">
              <option value="" disabled selected>Selecione o postinho em Marabá</option>
              <option value="ubs_hiroshi">UBS Hiroshi Matsuda (Folha 11)</option>
              <option value="ubs_laranjeiras">UBS Laranjeiras</option>
              <option value="ubs_amadeu">UBS Amadeu Vivácqua (São Félix)</option>
              <option value="ubs_carlos">UBS Carlos Barreto (Morada Nova)</option>
              <option value="ubs_zezinha">UBS Enfermeira Zezinha (Folha 23)</option>
              <option value="ubs_joao">UBS João Batista Bezerra (Marabá Pioneira)</option>
              <option value="ubs_jaime">UBS Jaime Pinto (Belo Horizonte)</option>
              <option value="ubs_pedro">UBS Pedro Cavalcante (Cidade Nova)</option>
              <option value="ubs_demosthenes">UBS Demósthenes Azevedo (Marabá Pioneira)</option>
              <option value="ubs_mariana">UBS Mariana Rocha (Novo Horizonte)</option>
              <option value="ubs_maria">UBS Maria Bico Doce (São Félix II)</option>
              <option value="ubs_emerson">UBS Emerson Caselli (Liberdade)</option>
  ...
  handleFormSubmit(disease, form) {
    const data = {
      ...
      healthUnit: document.getElementById(`health-unit-${disease}`).value,
    ...
    // Validate
    if (!data.patientName || !data.patientAge || !data.location || !data.healthUnit || !data.symptomsDate || !data.mainSymptoms) {
