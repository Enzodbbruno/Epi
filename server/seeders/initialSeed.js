...
async function seed() {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, 'admin', ?, 'Secretaria Municipal de Saúde')
  `).run(
    uuid(),
  ...
  const insertPatient = db.prepare(`
    ...
    VALUES
      (@id,@name,@cpfHash,@cpfEnc,@cns,@birthDate,@motherName,@sex,@race,
       @bloodType,@education,@zone,@address,@neighborhood,@municipality,@uf,@phone)
  `);
  ...
  const seedAll = db.transaction(() => {
    for (const p of samplePatients) {
      insertPatient.run({
        id:           uuid(),
        name:         p.name,
        cpfHash:      hashIndex(p.cpf),
        cpfEnc:       encrypt(p.cpf),
        cns:          p.cns,
